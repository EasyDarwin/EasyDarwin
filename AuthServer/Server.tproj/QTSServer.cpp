...
Bool16 QTSServer::CreateListeners(Bool16 startListeningNow, QTSServerPrefs* inPrefs, UInt16 inPortOverride)
{
    ...
    for (UInt32 count6 = 0; count6 < fNumListeners; count6++)
    {
        if  (fListeners[count6]->GetLocalAddr() != INADDR_LOOPBACK)
        {
            ...
            (void)this->SetValue(qtssSvrServerPorts, portIndex, &thePort, sizeof(thePort), QTSSDictionary::kDontObeyReadOnly);
            portIndex++;
        }
    }
...
{       
    ...
    QTSS_Error theErr = QTSS_NoErr;
    theErr = inPrefs->GetValue(qtssPrefsServicePort, 0, &thePort ,&theLen);
...
Task*   ServiceListenerSocket::GetSessionTask(TCPSocket** outSocket)
{
    ...
    CHttpSession* theTask = NEW CHttpSession();
