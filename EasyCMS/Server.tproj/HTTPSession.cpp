...
QTSS_Error HTTPSession::ExecNetMsgSnapUpdateReq(const char* json)//设备快照请求
{
	...
	string strTime			=	parse.GetBodyValue("Time");//时间属性有什么用呢
...
//虽然可以在里面进行处理，但还是放到ExecNetMsgStreamStopReq中,只保留一份处理
{
	...
	*/
	...
	EasyDarwin::Protocol::EasyProtocol req(MSG_CS_FREE_STREAM_REQ);//由restful接口合成json格式请求
	req.SetHeaderValue("CSeq","1");//CSeq是客户端直播的关键字，重复问题？
...
QTSS_Error HTTPSession::ExecNetMsgStreamStartReqRestful(char *queryString)//放到ProcessRequest所在的状态去处理，方便多次循环调用
{
	...
	*/
	...
	EasyDarwin::Protocol::EasyProtocol req(MSG_CS_GET_STREAM_REQ);//由restful接口合成json格式请求
	req.SetHeaderValue("CSeq","1");//CSeq是客户端直播的关键字，重复问题？
	req.SetHeaderValue("Version","1.0");
	req.SetBodyValue("Serial",chSerial);
	req.SetBodyValue("Channel",chChannel);
...
QTSS_Error HTTPSession::ExecNetMsgStreamStartReq(const char* json)//客户端开始流请求
{
	...
	*/
	...
	if(strDeviceSerial.size()<=0||strCameraSerial.size()<=0||strProtocol.size()<=0)//参数判断,不对strStreamID进行判断
		return QTSS_BadArgument;
...
QTSS_Error HTTPSession::ExecNetMsgGetCameraListReqJsonEx(const char* json)
{
	...
	*/
	...
	if(theDevRef==NULL)//不存在指定设备
	{
		return EASYDARWIN_ERROR_DEVICE_NOT_FOUND;//交给错误处理中心处理
	}
	else//存在指定设备，则获取这个设备的摄像头信息
	{
		Json::Value *proot=rsp.GetRoot();
