...
CHttpClient::CHttpClient(ClientSocket* inSocket, Bool16 verbosePrinting)
:	fSocket(inSocket),
...
void CHttpClient::Set(const StrPtrLen& inURL)
{
	delete [] fURL.Ptr;
...
OS_Error CHttpClient::SendHttpPacket(char *respXML)
{
...
OS_Error CHttpClient::SendPacket(char *resXML)
{
	OS_Error theErr = OS_NoErr;
	
	//qtss_sprintf(fMethod,"%s","POST");

	UInt32 contentLen = strlen(resXML);
	...
	fmt.PutFmtStr(
			...
			"CSeq: %d\r\n"
			"Content-Length: %d\r\n",
			fURL.Ptr == NULL? "www.easydarwin.org/libEasyCMS":fURL.Ptr, fCSeq, contentLen);
		
	fmt.PutFmtStr("\r\n");
	fmt.PutFmtStr(resXML);
	fmt.PutTerminator();
...
OS_Error CHttpClient::DoTransaction()
{
		OS_Error theErr = OS_NoErr;
		StrPtrLen theRequest(fSendBuffer, ::strlen(fSendBuffer));
...
OS_Error CHttpClient::ReceiveResponse()
{
...
*/
OS_Error CHttpClient::ReceivePacket()
{
		OS_Error theErr = OS_NoErr;
...
OS_Error	CHttpClient::SendBytes(char* bytes, unsigned int len)
{
	if(len == 0)
		return -10;
