...
// 上传Camera快照
static QTSS_Error Post_EasyCMSModule(Easy_CameraSnap_Params* inParams);
...
QTSS_Error EasyCMSModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
		...
		case Easy_GetCameraSnap_Role:
			return Post_EasyCMSModule(&inParams->cameraSnapParams);
	}
    return QTSS_NoErr;
}
...
QTSS_Error Post_EasyCMSModule(Easy_CameraSnap_Params* inParams)
{
